...
const mapSearch = (req,res)=>{
    try {
        ...
        let animalData=fs.readFileSync(path.join(__dirname,'../public/Animal.json'),'utf-8')
...
const mapInsert = (req,res)=>{  
    try {
        console.log("insert")
        let allBoundFileData=fs.readFileSync(path.join(__dirname,'../public/RTree.json'),'utf-8')
        let animalFileData=fs.readFileSync(path.join(__dirname,'../public/Animal.json'),'utf-8')
        ...
        let newBounds=RTree.instruction.insert(allBound,location,allBound.nodesNum++)
       
        let writeTree=JSON.stringify(newBounds,null,"\t")
        let writeAnimal=JSON.stringify(animalData,null,"\t")
        fs.writeFileSync(path.join(__dirname,'../public/RTree.json'),writeTree,'utf-8')
        fs.writeFileSync(path.join(__dirname,'../public/Animal.json'),writeAnimal,'utf-8')
        // let root=allBound.find(bound=>bound.type==="root")
...
const mapCreate=(req,res)=>{
    ...
    try {
        ...
        let animal_data=fs.readFileSync(path.join(__dirname,'../public/Animal.json'),'utf-8')
